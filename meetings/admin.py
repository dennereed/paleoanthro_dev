...
def create_abstract_csv(modeladmin, request, queryset):
    ...
    writer.writerow(['id', 'contact_email', 'presentation_type', 'title', 'abstract_text', 'acknowledgements', 'references', 'comments', 'funding', 'year', 'abstract_rank', 'authors'])
...
def create_abstract4publication_html(modeladmin, request, queryset):
    abstracts=queryset
    for abstract in abstracts:
        abstract.authors = abstract.author_set.all().order_by("author_rank")
        try:
            lead_last = abstract.authors[0].name.split()[-1]  # get the lead authors last name
        except IndexError:
            lead_last = abstract.authors[0].name.split()[0]  # get the last name if there is only one name
        abstract.lead_last_name = lead_last

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="PaleoAnthro_abstracts_for_publication.html"'
    t = loader.get_template("paleoanthro/abstract_publication_template.html")
    c = Context({
        'data': abstracts,
    })
    response.write(t.render(c))
    return response
create_abstract4publication_html.short_description = "Download .html for publication"
